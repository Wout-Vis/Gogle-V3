...
export default class ControlPanel extends Component {
  ...
  updateSimulationValues = () => {
    ...
    passValue[simEconomy] =
      0.15 * cfg[cfgConsumer] +
      0.2 * cfg[cfgPolitics] +
      0.15 * cfg[cfgConflict] +
      0.4 * cfg[cfgEconomy];
    ...
    passValue[simProgress] =
      ...
      0.35 * cfg[cfgResearch] +
      0.15 * cfg[cfgPolitics] +
      0.15 * cfg[cfgEconomy];
    passValue[simPolitics] =
      0.5 * cfg[cfgPolitics] + 0.25 * cfg[cfgConflict] + 0.25 * cfg[cfgEconomy];
  ...
  getRandomMessage() {
    ...
    let messageIndex = Math.round(
      catIndex * 10 + (catValue + 50) / 10 + (Math.random() * 4 - 2)
    );
  ...
  ending(simValues) {
    ...
    $("#cpBody").fadeOut(6000, function() {
      ...
      for (let i = 0; i < simValues.length; i++) {
        if (Math.abs(simValues[i]) >= Math.abs(simValues[highestSV]))
          highestSV = i;
      }
      ...
      toast("BREAKING NEWS: " + headlineArray[headlineIndex], {
        position: "top-right",
        type: "warning",
        autoClose: false,
  ...
  componentDidMount() {
    this.logTicker = setInterval(
      () => this.updateLogText(this.getRandomMessage()),
      2000
    );
  }
      ...
      <div>
        <div id="cpBody" className="controlPanelBG">
          ...
          <div id="panelBox">
            ...
            <div
              id="logBox"
              dangerouslySetInnerHTML={{ __html: this.state.logText.join("") }}
            ></div>
          </div>
          <button
            id="applyButton"
            onClick={() => this.ending(this.state.simulationValues)}
          >
            apply all
          </button>
        </div>
